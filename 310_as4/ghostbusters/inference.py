...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        sum = .0
        for key in self:
            if sum <= probability < sum + self[key]:
                return key
            sum += self[key]
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for position in self.allPositions:
            tempDist[position] = 0
        for gamePosition in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, gamePosition)
            for newPostion in newPosDist:
                tempDist[newPostion] += newPosDist[newPostion] * self.beliefs[gamePosition]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for state in ghostStates:
            for index in range(self.numGhosts):
                jailPosition = self.getJailPosition(index)
                observationIndex = observation[index]
                ghostPosition = state[index]
                predictions[state] *= self.getObservationProb(observationIndex, pacmanPosition, ghostPosition, jailPosition)
        predictions.normalize()
        if predictions.total() == 0:
            self.initializeUniformly(gameState)
        else:
