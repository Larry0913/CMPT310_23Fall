    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not hitsWall:
                ...
                if nextState in self.corners and nextState not in nextCornersVisited:
                    nextCornersVisited.append(nextState)
                successors.append(((nextState, tuple(nextCornersVisited)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for corner_position in corners:
        if corner_position not in cornerVisited:
            cornerList.append(corner_position)

    while len(cornerList) != 0:
        temp = None
        min = 99999
        for corner in cornerList:
            dist = util.manhattanDistance(currentState, corner)
            if min > dist:
                min = dist
                temp = corner
        costTotal += min
        cornerList.remove(temp)
        currentState = temp

    return costTotal  # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    if not foods:
        return 0  # No food left, no cost to reach the goal
    max_distance = 0
    for food in foods:
